...
// StructToMap ...
func StructToMap(s any, p Map) error {
	var err error
...
/*ToMap transfer to map[string]any or MapAble to GMap  */
...
func (m Map) SetNil(s string, v any) Map {
	if !m.Has(s) {
		m.Set(s, v)
	}
	return m
...
func (m Map) SetHas(s string, v any) Map {
	if m.Has(s) {
		m.Set(s, v)
	}
	return m
		...
		case xml.EndElement:
			...
			if last == arrayTag {
				...
				if arr != nil {
					...
					//exception doing
					maps.Set(arrayTag, []any{maps.Get(arrayTag)})
				}
...
func convertXML(k string, v any, e *xml.Encoder, start xml.StartElement) error {
	var err error
	switch v1 := v.(type) {
	case Map:
