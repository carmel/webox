...
type Cipher interface {
	Type() CryptType
	Encrypt(any) ([]byte, error)
	Decrypt(any) ([]byte, error)
}
...
func parseBizMsg(data any) (d *BizMsgData, e error) {
	switch tmp := data.(type) {
	case *BizMsgData:
		d = tmp
	case BizMsgData:
	...
	default:
		e = errors.New("wrong type inputed")
	}
	return
}
